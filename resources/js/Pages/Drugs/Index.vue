...
const tableColumns = computed(() => {
  switch (activeFilter.value) {
    case 'sales':
      return [
        { key: 'id', label: '#', formatter: true },
        { key: 'name', label: 'Drug' },
        { key: 'category', label: 'Category' },
        { key: 'quantity', label: 'Quantity' },
        { key: 'price', label: 'Price', formatter: true },
      ]
    case 'low_stock':
      return [
        { key: 'id', label: 'ID', formatter: true },
        { key: 'name', label: 'Drug' },
        { key: 'generic_name', label: 'Generic Name' },
        { key: 'quantity', label: 'Qty Left' },
        { key: 'expiry_date', label: 'Expiry Date', formatter: true },
      ]
    case 'expiring_soon':
    case 'expired':
      return [
...
const columnFormatters: Record<string, (d: Drug, i?: number) => any> = {
  ...
  price: (d: Drug) => (d.price != null ? `₦${Number(d.price).toLocaleString()}` : '—'),
  expiry_date: (d: Drug) => (d.expiry_date ? new Date(d.expiry_date).toLocaleDateString() : '—'),
}

/* Check if a column has a formatter */
function hasFormatter(key: string): boolean {
  return key in columnFormatters
}

/* Status badge generator */
function getStatusBadge(drug: Drug) {
  if (drug.expiry_date && new Date(drug.expiry_date) < new Date()) {
    return { class: 'bg-red-100 text-red-700', text: 'Expired' }
  }
  if (drug.quantity === 0) {
    return { class: 'bg-gray-100 text-gray-700', text: 'Out of Stock' }
  }
  if (drug.quantity && drug.quantity < 10) {
...
/* Helper to safely access drug properties */
function getDrugValue(drug: Drug, key: string): any {
  ...
  <AppLayout title="Drug Inventory">
    <div class="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div class="flex-1 max-w-md">
          <div class="relative">
            <input
              ...
              class="w-full pl-4 pr-12 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-teal-400"
            />
            <button
              @click="handleSearch"
              class="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-teal-500 rounded-full flex items-center justify-center text-white"
            >
      ...
      <div class="bg-white rounded-xl shadow overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr
                v-for="(drug, rowIndex) in props.drugs.data"
                :key="drug.id"
              ...
              >
                ...
                >
                  <template v-if="hasFormatter(col.key) && columnFormatters[col.key]">
                    {{ columnFormatters[col.key]!(drug, rowIndex) }}
                  </template>

                  <template v-else-if="col.key === 'status'">
                    <span
                      :class="getStatusBadge(drug).class"
                ...
                >
                  <div class="flex flex-col items-center gap-2">
                    <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6"/>
                    </svg>
        ...
        <!-- pagination -->
        <div
          v-if="props.drugs.data?.length"
          class="px-6 py-4 border-t border-gray-200 flex items-center justify-between"
        >
          <div class="text-sm text-gray-600">
            Showing {{ props.drugs.data.length }} of {{ pagination.total }} results
          </div>
          <div class="flex gap-2">
            <Link
              v-if="pagination.current_page > 1"
              :href="route('drugs.index', { page: pagination.current_page - 1 })"
              class="px-4 py-2 border rounded-lg text-sm text-gray-700"
            >
              Previous
            </Link>
            <Link
              v-if="pagination.current_page < (props.drugs.last_page ?? pagination.last_page)"
              :href="route('drugs.index', { page: pagination.current_page + 1 })"
              class="px-4 py-2 border rounded-lg text-sm text-gray-700"
            >
              Next
